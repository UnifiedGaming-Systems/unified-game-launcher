...
class GOGAPI:
    def __init__(self, client_id: Optional[str] = None, client_secret: Optional[str] = None):
        ...
        self.gog_path = self._find_gog_patch()
    ...
    def _find_gog_path(self) -> Optional[Path]:
        ...
        try:
            if os.name == 'nt' # Windows
                key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\WOW6432\GOG.com\GalaxyClient")
            ...
            self.logger.error(f"Failed to find the GOG Galaxy Path: {e}")
        return None
    ...
    def _start_auth_server(self) -> tuple[HTTPServer, int]:
        ...
        while port < 9000:
            try:
                ...
                port += 1
        raise Exception("No available ports found for the auth server")
    
    def _get_tokens(self, auth_code: str, redirect_uri: str) -> bool:
        """Exchange authorization code for acces and refresh tokens"""
        try:
            data = {
                'client_id': self.client_id,
    ...
    def refresh_auth_token(self) -> bool:
        ...
        try:
            ...
            tokens = response.json()
            self.access_token = tokens.get('access_token')
            self.refresh_token = tokens.get('refresh_token') # New refresh rate
